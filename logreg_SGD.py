...
def logreg(data_path, lamda, delta, epsilon, norm_bound, learning_rate, use_cross_validation):
    # get data
    ...
    if use_cross_validation:
        ...
        for h in range(5):
            ...
            # train
            lr.fit_SGD_noise(training_data, training_label,delta, epsilon, norm_bound,learning_rate)
        ...
        for X_train, y_train, X_test, y_test in datasets:
            lr = logreg_model.Logreg(num_features, lamda)
            # train
            lr.fit_SGD_noise(X_train, y_train,delta, epsilon, norm_bound,learning_rate)
...
if __name__ == '__main__':
    ...
    parser.add_argument('delta', metavar='delta', type=float, help='delta')
    parser.add_argument('epsilon', metavar='epsilon', type=float, help='epsilon')
    parser.add_argument('norm_bound', metavar='norm_bound', type=float, help='norm_bound')
    ...
    if args.delta < 0:
        raise argparse.ArgumentTypeError('delta must be an positive number')
    # if epsilon is negative
    if args.epsilon < 0:
        raise argparse.ArgumentTypeError('epsilon must be an positive number')
    # if norm bound is negative
    if args.norm_bound < 0:
        raise argparse.ArgumentTypeError('norm_bound must be an positive number')
    ...
    logreg(data_path, lamda, delta,epsilon, norm_bound, learning_rate, use_cross_validation)
